    ...
    var rows = sortData.map(item => (
      <tr key={item.id}>
        ...
        <td>
          <Button
            variant="success"
            onClick={() => this.handleToUpdatePage(item)}
          >
            編集
          </Button>
          <span>&nbsp;</span>
          <Button
            variant="danger"
            onClick={() => this.openDeleteModal(item.id, item.title)}
          >
            削除
          </Button>
          ...
          <Modal
            ...
            onHide={() => this.closeModal(this.DELETE_CODE)}
          >
            ...
            <Modal.Body>
              <h3>
                {this.state.deleteId} {this.state.deleteTitle}
              </h3>
              <h3>このTodoを削除しますか？</h3>
            </Modal.Body>
            <Modal.Footer>
              <Button onClick={this.delete}>はい</Button>
              <Button onClick={() => this.closeModal(this.DELETE_CODE)}>
                いいえ
              </Button>
            </Modal.Footer>
          </Modal>
